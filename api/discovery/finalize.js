...
/** Build the artifacts from a (possibly LLM-compiled) partial brain.
...
/**
 * Close a session: compile, build artifacts, persist, notify.
 *
 * Lives apart from the HTTP handler because the interview must also be able
 * to close ITSELF. A prospect who is told "listo, te contactamos en menos de
 * 24 horas" has no reason to then hunt for a Terminar button — and the ones
 * who didn't press it were silently lost.
 *
 * Returns { ok } or { ok:false, error:'email_required'|... }. Never throws.
 */
async function finalizeSession(s) {
  ...
  if (!brainLib.hasEmail(partial)) {
    ...
    return { ok: false, error: 'email_required', message: forClient(clientKey).EMAIL_REQUIRED_MESSAGE };
  }
  ...
  try { await store.save(s); } catch (e) { return { ok: false, error: 'store_unavailable_on_save' }; }
...
module.exports = async function handler(req, res) {
  ...
  if (out.error === 'email_required') return res.status(400).json(out);
