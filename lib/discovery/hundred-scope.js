...
/** Channels the agent would actually have to cover.

    Two kinds get dropped, because counting them inflates the difficulty and
    therefore the price:
    - the physical counter (mostrador/presencial), which no agent attends;
    - channels the prospect described as dead ("el Instagram lo tenemos
      abandonado"), which are not work to be done. */
...
function channelSet(brain) {
  ...
  // Pair each channel with whatever the prospect said about its volume, so a
  // channel described as abandoned can be recognised as dead.
  const entries = [
...
function assessDifficulty(brain, rec) {
  ...
  // Atender dudas es parte del mismo agente cuando ya toma pedidos o citas: no
  // se cobra dos veces. Solo suma un agente de naturaleza distinta (p. ej. leads).
  const extraAgents = rec.secondary.filter((s) => s.key !== 'front_desk');
  if (extraAgents.length) { points += 1; reasons.push('Requiere un segundo agente de naturaleza distinta.'); }
