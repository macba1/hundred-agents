/* ============================================================
   ...
   This file holds the ORIGINAL Gabi profile (section list, system
   prompt, update_brain tool schema, compile prompt) and exports it
   both as flat named exports — kept for backwards compatibility with
   existing imports and tests — and through forClient(), the per-client
   registry used by the API handlers.

   A second profile, "hundred" (the generic Hundred Agents diagnostic
   served at /diagnostico), lives in ./prompts-hundred.js. Adding a
   client means adding a profile file and one line in PROFILES.
   ============================================================ */
...
/* ---- Per-client profile registry ---------------------------------- */

// Asked by /api/discovery/finalize when the email is still missing.
const EMAIL_REQUIRED_MESSAGE = '¿Cuál es el mejor email para enviarte la propuesta de implantación y comercial con precios?';

const GABI_PROFILE = { SECTIONS, SYSTEM, UPDATE_BRAIN_TOOL, COMPILE_SYSTEM, GREETING, FINAL_MESSAGE, EMAIL_REQUIRED_MESSAGE };

const PROFILES = {
  gabi: GABI_PROFILE,
  hundred: require('./prompts-hundred'),
};

const DEFAULT_CLIENT = 'gabi';

/** Prompt profile for a clientKey. Unknown keys fall back to Gabi so a
    mis-typed key can never leave a handler without a system prompt. */
function forClient(clientKey) {
  return PROFILES[clientKey] || PROFILES[DEFAULT_CLIENT];
}

/** Client keys that have a prompt profile (i.e. can actually run). */
function knownClients() {
  return Object.keys(PROFILES);
}

module.exports = {
  SECTIONS, SYSTEM, UPDATE_BRAIN_TOOL, COMPILE_SYSTEM, GREETING, FINAL_MESSAGE,
  EMAIL_REQUIRED_MESSAGE, forClient, knownClients, PROFILES, DEFAULT_CLIENT,
};
