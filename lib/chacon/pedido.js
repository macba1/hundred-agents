 ...
 */
async function confirmar(clienteId, {
  clave_idempotencia = null, observaciones = null, telefono = null } = {}) {
  ...
  /* Foto fija de si el teléfono que pide estaba verificado EN ESTE MOMENTO.
     No se recalcula nunca: que Chacón verifique el número mañana no cambia
     el hecho de que este pedido salió sin comprobar. Un pedido es el
     registro de lo que pasó. */
  const verificacion = await confianza.evaluar(
    telefono || cliente.telefonos?.[0] || null, cliente);
  const totales = precios.totalizar(carrito.lineas);
  const pedido = {
