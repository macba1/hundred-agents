/* ============================================================
   ...
   Estados y qué significa estar en cada uno:
     ...
     PRIVACY_ONBOARDING       se ha informado del uso de datos y se espera
                                que confirme que quiere seguir por aquí
...
const ESTADOS = [
  ...
  'PRIVACY_ONBOARDING', 'CUSTOMER_IDENTIFICATION', 'ORDER_COMPLETE',
  'HUMAN_HANDOFF',
];

/**
 * Ciclo de vida del pedido. Un pedido CONFIRMADO deja de ser carrito activo:
 * confundirlos hizo que a Fernando le apareciera un pedido viejo al pedir uno
 * nuevo.
 */
const PEDIDO = {
  DRAFT: 'DRAFT',                       // se está construyendo
  CHECKOUT_PENDING: 'CHECKOUT_PENDING', // resumen enseñado, falta confirmar
  CONFIRMED: 'CONFIRMED',               // cerrado e inmutable
  CANCELLED: 'CANCELLED',               // cancelado a propósito
  ABANDONED: 'ABANDONED',               // sustituido por otro
};

/** Solo estos pueden ser el borrador activo. */
const ACTIVOS = new Set([PEDIDO.DRAFT, PEDIDO.CHECKOUT_PENDING]);
...
/* Desde CUALQUIER estado se puede tener que identificar al cliente, y desde
   ...
   nuevo. */
const SIEMPRE = ['PRIVACY_ONBOARDING', 'CUSTOMER_IDENTIFICATION', 'HUMAN_HANDOFF', 'HOME'];
...
const TRANSICIONES_BASE = {
  ...
  REORDER: ['CART', 'CHECKOUT', 'CART_EDIT', 'PRODUCT_DISCOVERY', 'PRODUCT_SELECTION',
            'HOME', 'HUMAN_HANDOFF'],
  CHECKOUT: ['CONFIRMATION', 'ORDER_COMPLETE', 'CART', 'CART_EDIT',
             'PRODUCT_DISCOVERY', 'HOME'],
  ORDER_COMPLETE: ['PRODUCT_DISCOVERY', 'FAMILY_SELECTION', 'PRODUCT_SEARCH',
                   'REORDER', 'HOME'],
  CONFIRMATION: ['ORDER_COMPLETE', 'HOME', 'PRODUCT_DISCOVERY', 'REORDER'],
  ...
  /* Desde el aviso solo se puede ir a identificar, volver a donde se estaba,
     o quedarse. Nunca directamente a comprar: primero hay que decir que sí. */
  PRIVACY_ONBOARDING: ESTADOS.filter((e) => e !== 'CONFIRMATION' && e !== 'CHECKOUT'),
};
...
module.exports = {
  ESTADOS, TRANSICIONES, TRANSICIONES_BASE, SLOTS, SIEMPRE, PEDIDO, ACTIVOS,
