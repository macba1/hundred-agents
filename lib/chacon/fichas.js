...
async function revision(codigo) {
  try {
    ...
    /* Si el almacén no responde no se puede saber qué está validado, así que
       no se enseña nada: se cae al PDF, que es la respuesta segura y no
       depende de Redis. Fallar hacia "no tengo el dato" es correcto; fallar
       con una excepción dejaría a la tienda sin respuesta ninguna. */
    console.warn('[chacon] sin revisión de fichas (%s): se responde solo con el PDF',
 ...
 */
/* Ingredientes y alérgenos se responden el uno por el otro cuando falta uno.
   Los dos son texto literal del mismo documento y contestan la misma
   pregunta: el fabricante a veces declara los alérgenos dentro de la lista de
   ingredientes y no en un apartado aparte. */
const RESPALDO = { alergenos: 'ingredientes', ingredientes: 'alergenos' };
...
async function consultar(codigo, campo) {
  ...
  const alternativo = RESPALDO[campo];
  if (alternativo && visibles[alternativo]) {
    return { hay: true, campo: alternativo, etiqueta: CAMPOS[alternativo],
             texto: visibles[alternativo].texto, por_respaldo: campo,
             archivo: f.archivo, pdf, codigo: cod };
  }
...
/**
 * Todo lo que se puede decir de un producto, para "dame la ficha técnica".
 *
 * Solo campos validados. Si no hay ninguno, no se devuelve un resumen vacío
 * con pinta de respuesta: se dice que no hay datos sueltos y se manda el
 * documento, que es la respuesta correcta.
 */
async function consultarTodo(codigo) {
  const cod = normalizarCodigo(codigo);
  const f = porCodigo(cod);
  if (!f) return { hay: false, motivo: 'sin_ficha', pdf: false, campos: [], codigo: cod };

  const visibles = await camposVisibles(cod);
  const campos = Object.keys(CAMPOS)
    .filter((c) => visibles[c])
    .map((c) => ({ campo: c, etiqueta: CAMPOS[c], texto: visibles[c].texto }));

  return {
    hay: campos.length > 0,
    motivo: campos.length ? null : (f.sin_capa_texto ? 'ficha_escaneada' : 'sin_campos_validados'),
    campos,
    pdf: tienePdf(cod),
    archivo: f.archivo,
    codigo: cod,
...
module.exports = {
  ...
  revision, camposVisibles, consultar, consultarTodo, textoSinDato, resumen, recargar,
};
