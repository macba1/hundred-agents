...
async function responderFicha(telefono, producto, campo) {
  ...
  /* La pregunta queda contestada: si se deja anotada, el siguiente mensaje
     se interpretaría otra vez como si preguntara lo mismo. */
  const { maquina } = await estados.leer(telefono);
  const datos = { ...maquina.datos, codigo: producto.codigo };
  delete datos.ficha_pedida;
  await estados.mover(telefono, 'QUANTITY_SELECTION', datos,
...
async function porTexto(telefono, cliente, textoCrudo) {
  ...
  /* Pregunta sobre un producto. Va DESPUÉS de las intenciones globales
