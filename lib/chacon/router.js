 ...
 */
async function pedirPrivacidad(telefono, { accionPendiente = null } = {}) {
  const { maquina } = await estados.leer(telefono);
  await estados.mover(telefono, 'PRIVACY_ONBOARDING',
    { ...maquina.datos, ...(accionPendiente ? { accion_pendiente: accionPendiente } : {}) },
...
async function volverDeIdentificacion(telefono, cliente) {
  ...
  const saludo = formato.texto(`Perfecto, ${cliente.nombre}. Seguimos con tu pedido.`);
...
async function resolverNegocio(telefono, texto) {
  ...
  if (!nombre && /^[A-Za-z0-9-]{3,12}$/.test(String(texto).trim())) {
    const porCod = await identificacion.porNombre(String(texto).trim());
    if (porCod.estado === 'encontrado') {
      return { cliente: porCod.cliente,
               pantallas: await volverDeIdentificacion(telefono, porCod.cliente) };
    }
  ...
  const r = await identificacion.porNombre(nombre);
  if (r.estado === 'encontrado') {
    // El teléfono se asocia para no volver a preguntar nunca más.
    if (!(r.cliente.telefonos || []).includes(telefono)) {
      r.cliente.telefonos = [...(r.cliente.telefonos || []), telefono];
      await repo.guardarCliente(r.cliente);
    }
    return { cliente: r.cliente, pantallas: await volverDeIdentificacion(telefono, r.cliente) };
  }
  if (r.estado === 'ambiguo') {
    console.log('[chacon][evento] customer_identification_ambiguous tel=%s n=%s',
      telefono, r.candidatos.length);
    const { maquina: mAmb } = await estados.leer(telefono);
    await estados.mover(telefono, 'CUSTOMER_IDENTIFICATION',
      { ...mAmb.datos, candidatos_cliente: r.candidatos.map((c) => c.id) },
      { motivo: 'varios negocios parecidos', slot: estados.SLOTS.BUSINESS_NAME,
        conservarPrevio: true });
    return flujo.negocioAmbiguo(r.candidatos);
  }
  ...
  await estados.mover(telefono, 'CUSTOMER_IDENTIFICATION',
    ...
    { motivo: 'negocio no encontrado', slot: estados.SLOTS.BUSINESS_NAME,
      conservarPrevio: true });
  return flujo.negocioNoEncontrado(nombre, { altaLibre: identificacion.altaLibre() });
}

/**
 * Continúa con el nombre que dio, dándolo de alta. **Solo en modo demo**
 * (`CHACON_ALTA_LIBRE=1`): fuera de la demo este botón no se ofrece, y si
 * llegara igualmente se rechaza aquí. No basta con no enseñarlo.
 */
async function altaDeNegocio(telefono) {
  if (!identificacion.altaLibre()) {
    console.warn('[chacon] alta_negocio con CHACON_ALTA_LIBRE apagado: se ignora');
    return pedirIdentificacion(telefono, { reintento: true });
  }
  const { maquina } = await estados.leer(telefono);
  const nombre = maquina.datos.ultimo_nombre;
  if (!nombre) return pedirIdentificacion(telefono, { reintento: true });

  const cliente = await identificacion.registrar(nombre, telefono);
  console.log('[chacon][evento] customer_identification_registered tel=%s cliente=%s nombre=%j modo=demo',
    telefono, cliente.id, nombre);
  return volverDeIdentificacion(telefono, cliente);
}
...
async function porClic(telefono, cliente, id) {
  ...
  if (id === 'alta_negocio') return altaDeNegocio(telefono);
...
async function porTexto(telefono, cliente, texto) {
  ...
  if (/^(menu|inicio|volver|atras)$/.test(q)) {
    await estados.mover(telefono, 'HOME', {}, { motivo: 'volver' });
    return flujo.home(cliente);
  }
  // Escrito o pulsado, "ver familias" hace exactamente lo mismo.
  if (/^(ver\s+)?(otras\s+)?familias?$|^categorias?$|^ver\s+catalogo$/.test(q)) {
    await estados.entrarEnFamilias(telefono, { motivo: 'familias por texto' });
    return flujo.menuFamilias();
  }
  if (/(ver|mi|el)?\s*(pedido|carrito)$/.test(q) && clienteId) {
    await estados.mover(telefono, 'CART', {}, { motivo: 'ver carrito' });
    return flujo.verCarrito(clienteId);
  }
  if (/^(ofertas?|que ofertas|promociones?)/.test(q)) {
    await estados.mover(telefono, 'OFFERS', {}, { motivo: 'ofertas' });
    const filtro = q.replace(/^(que )?(ofertas?|promociones?)( de| en)?/, '').trim();
    return flujo.verOfertas({ filtro: filtro || null });
  }
  if (/ofertas? (de|en) /.test(q)) {
    await estados.mover(telefono, 'OFFERS', {}, { motivo: 'ofertas filtradas' });
    return flujo.verOfertas({ filtro: q.split(/ofertas? (?:de|en) /)[1] });
  }
...
module.exports = {
  ...
  pedirIdentificacion, resolverNegocio, volverDeIdentificacion, altaDeNegocio, pedirPrivacidad, aceptarPrivacidad,
  NUMEROS, ORDINALES,
};
