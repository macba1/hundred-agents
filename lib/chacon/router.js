 ...
 */
async function pedirIdentificacion(telefono, { reintento = false } = {}) {
...
async function resolverNegocio(telefono, texto) {
  const nombre = identificacion.nombreDeNegocio(texto);
  if (!nombre) {
...
async function porClic(telefono, cliente, id) {
  ...
  if (id.startsWith('cant:')) {
    ...
    if (!clienteId) return null;         // hace falta identificar la tienda
    const { maquina } = await estados.leer(telefono);
