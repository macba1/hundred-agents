...
const confianza = require('./confianza');
...
/**
 * Contesta lo que se preguntó de un producto. Un solo sitio, para que dé
 * igual si se llegó escribiendo el nombre, tocando una opción o diciendo
 * "el segundo".
 */
async function responderFicha(telefono, producto, campo) {
  const res = campo
    ? await fichas.consultar(producto.codigo, campo)
    : await fichas.consultarTodo(producto.codigo);
  const url = fichas.urlPdf(producto.codigo);

  /* El tema NO se borra al contestar. "y el queso?" después de preguntar por
     el lomo sigue siendo la misma pregunta, y borrarlo hacía que la segunda
     vez saliera la tarjeta de precios. Se limpia cuando el cliente pasa a
     comprar de verdad —añadir al carrito o empezar un pedido—, que es cuando
     deja de estar preguntando. */
  const { maquina } = await estados.leer(telefono);
  await estados.mover(telefono, 'QUANTITY_SELECTION',
    { ...maquina.datos, codigo: producto.codigo, ficha_pedida: { campo } },
    { motivo: 'ficha contestada' });

  console.log('[chacon][evento] ficha_consultada tel=%s codigo=%s campo=%s hay=%s pdf=%s',
    telefono, producto.codigo, campo || 'todo', res.hay, !!url);
  return flujo.respuestaFicha(producto, res, { url, campo });
}
...
async function pedirIdentificacion(telefono, { reintento = false, accionPendiente = null } = {}) {
  /* Antes de preguntar nada: ¿dice Chacón de quién es este número? Es la
     única identificación que no depende de lo que cuente quien escribe, así
     que vale por sí sola y además ahorra la pregunta. Va aquí, en el único
     sitio por el que pasan todas las peticiones de identificación, y no
     repartida por los ocho puntos que llaman a esta función. */
  const deAgenda = agenda.porTelefonoAgenda(telefono);
  if (deAgenda) {
    const centro = agenda.centroDe(deAgenda);
    const ficha = await identificacion.vincular(deAgenda, telefono,
      { center: centro.center, estadoCentro: centro.estado, nivel: confianza.NIVELES.AGENDA });
    await privacidad.vincularCliente(telefono, ficha.id).catch(() => {});
    console.log('[chacon][evento] customer_identified_by_agenda_phone tel=%s code=%s',
      telefono, ficha.customer_code);
...
async function confirmar(telefono, cliente, { wamid = null } = {}) {
  ...
  const r = await pedidoLib.confirmar(cliente.id, { clave_idempotencia: clave, telefono });
  if (!r.ok) {
...
async function porClic(telefono, cliente, id) {
  ...
  if (id.startsWith('prod:')) {
    ...
    /* Si se llegó aquí preguntando algo —"¿cuál de estos?" era solo para
       saber de qué producto hablábamos—, se contesta la pregunta y no se
       enseña la tarjeta de compra. */
    const { maquina: mq } = await estados.leer(telefono);
    if (mq.datos.ficha_pedida) {
      return responderFicha(telefono, p, mq.datos.ficha_pedida.campo || null);
    }
...
/* ---- texto libre -------------------------------------------------------- */
async function porTexto(telefono, cliente, textoCrudo) {
  const clienteId = cliente ? cliente.id : null;

  /* El saludo de cortesía se recorta UNA vez, aquí, y lo que se clasifica es
     lo que viene detrás. La gente saluda y pide en el mismo mensaje: "hola,
     la caña de lomo lleva azúcar?" se trataba como un saludo a secas y se
     contestaba con el menú de inicio, porque el "hola" delante hacía que la
     frase entera dejara de parecer una pregunta.

     Se hace en el router y no en cada regla: si cada capa tiene que acordarse
     de quitar el saludo, alguna se olvidará. */
  const texto = intenciones.sinSaludo(textoCrudo) || textoCrudo;
  const saludaba = norm(textoCrudo) !== norm(texto);
  const q = norm(texto);
  const { maquina } = await estados.leer(telefono);
  ...
  /* Saludo y nada más: se vuelve al inicio. Si detrás del saludo venía una
     petición, ya se ha recortado arriba y aquí no queda saludo que ver. */
  if (saludaba && !q) {
  ...
  /* Una pregunta la contesta el MODELO, no esta capa.
     Aquí solo se sabe lo que pone el mensaje; el modelo tiene además la
     conversación entera, que es lo único que permite entender "y el queso?"
     detrás de "el lomo lleva azúcar?". Reconocer cada forma de preguntar con
     reglas exige una regla nueva cada vez —"azúcar" no estaba en ninguna
     lista— y siempre falta una.

     Lo que NO se le cede: el carrito, los precios, la identidad de la tienda
     y el estado del pedido. Eso lo sigue decidiendo el código, y el texto de
     una ficha sigue saliendo literal del documento. El modelo entiende; no
     afirma ni actúa por su cuenta. */
  if (process.env.CHACON_LLM_PREGUNTAS !== '0' && intenciones.esPregunta(texto)) {
    console.log('[chacon][intent] tel=%s pregunta -> agente estado=%s',
      telefono, maquina.estado);
    return null;
  }

  /* Respaldo determinista, por si se apaga lo anterior. Va DESPUÉS de las
     intenciones globales
     —"terminar pedido" manda sobre cualquier otra cosa— y ANTES del buscador,
     que era donde se perdía: "dame la descripción técnica de la caña de lomo"
     se buscaba entera en el catálogo. */
  const pregunta = intenciones.fichaPedida(texto);
  if (pregunta) {
    /* Sin producto en la frase —"¿lleva gluten?"— se pregunta por lo último
       que se enseñó. Si no hay nada, se pide el nombre en vez de adivinar. */
    if (!pregunta.producto) {
      const ultimo = maquina.datos.codigo
        || (maquina.datos.mostrados || []).length === 1 ? maquina.datos.codigo
          || maquina.datos.mostrados[0] : null;
      const p = ultimo && catalogo.todos().find((x) => x.codigo === ultimo);
      if (p) return responderFicha(telefono, p, pregunta.campo);
      return [formato.texto('¿De qué producto? Dime el nombre y te lo miro.')];
    }

    const res = descubrimiento.buscar(pregunta.producto);
    if (res.tipo === 'producto') {
      return responderFicha(telefono, res.producto, pregunta.campo);
    }
    if (res.tipo === 'varios' || res.tipo === 'familia') {
      /* Se anota QUÉ se preguntó antes de desambiguar. Sin esto, elegir una
         opción devolvía la tarjeta de compra y la pregunta se perdía: es el
         bug que vimos en producción. */
      const pantallas = await presentar(telefono, res,
        { titulo: '¿De cuál quieres la información?' });
      const { maquina: mq } = await estados.leer(telefono);
      await estados.mover(telefono, mq.estado,
        { ...mq.datos, ficha_pedida: { campo: pregunta.campo } },
        { motivo: 'pregunta pendiente de saber el producto', conservarPrevio: true });
      return pantallas;
    }
    return noEncontrado(telefono, pregunta.producto);
  }
  ...
  if (ord !== null && estados.ESPERAN_ORDINAL.has(maquina.estado)) {
    ...
    if (p) {
      if (maquina.datos.ficha_pedida) {
        return responderFicha(telefono, p, maquina.datos.ficha_pedida.campo || null);
      }
      await estados.mover(telefono, 'QUANTITY_SELECTION', { codigo: p.codigo },
