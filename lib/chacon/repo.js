/* ============================================================
   ...
   usa `wa:`):
     ...
     ch:ficha:rev:{codigo}           JSON con la revisión humana de cada
                                        campo de la ficha técnica. Vive
                                        aparte del fichero importado: el
                                        fichero es la fuente, esto es el
                                        trabajo de revisión, y reimportar no
                                        puede borrarlo
     ch:fichas:revisadas             SET de códigos con alguna revisión
     ch:reclama:{telefono}           SET de códigos de tienda que ese
                                        teléfono ha dicho ser. Más de uno es
                                        la señal de suplantación más clara
                                        que tenemos
   ============================================================ */
...
const K = {
  ...
  reclama: (tel) => `ch:reclama:${tel}`,
  revisionFicha: (cod) => `ch:ficha:rev:${cod}`,
  fichasRevisadas: 'ch:fichas:revisadas',
...
async function borrarCliente(id, { purgarPedidos = false } = {}) {
  ...
  for (const tel of ficha.telefonos || []) {
    await c.del(K.clientePorTel(tel));
    await c.sRem(K.reclama(tel), String(ficha.customer_code || ''));
  }
...
/**
 * Anota que este teléfono ha dicho ser esta tienda.
 *
 * Se registra SIEMPRE, aunque la identificación no acabe en pedido: alguien
 * probando nombres hasta acertar no llega a pedir, y es justo lo que hay que
 * poder ver después.
 */
async function anotarReclamacion(telefono, customerCode) {
  if (!telefono || !customerCode) return;
  const c = await cli();
  await c.sAdd(K.reclama(telefono), String(customerCode));
}

/** Tiendas distintas que este teléfono ha dicho ser. */
async function tiendasReclamadasPor(telefono) {
  if (!telefono) return [];
  return (await (await cli()).sMembers(K.reclama(telefono))) || [];
}

/* ---- fichas técnicas: revisión humana ---------------------------------- */
async function getRevisionFicha(codigo) {
  return leerJSON(await (await cli()).get(K.revisionFicha(codigo)));
}

/**
 * Guarda la decisión sobre UN campo. Se guarda campo a campo y no la ficha
 * entera: dos personas revisando a la vez no deben pisarse el trabajo.
 */
async function guardarRevisionCampo(codigo, campo, decision) {
  const c = await cli();
  const actual = (await getRevisionFicha(codigo)) || {};
  actual[campo] = { ...decision, ts: new Date().toISOString() };
  await c.set(K.revisionFicha(codigo), JSON.stringify(actual));
  await c.sAdd(K.fichasRevisadas, String(codigo));
  return actual;
}

async function fichasRevisadas() {
  return (await (await cli()).sMembers(K.fichasRevisadas)) || [];
}
...
module.exports = {
  ...
  anotarReclamacion, tiendasReclamadasPor,
  getRevisionFicha, guardarRevisionCampo, fichasRevisadas,
