...
/**
 * Un documento adjunto: la ficha técnica del fabricante.
 *
 * Se manda el PDF entero y sin tocar. Cuando no sabemos leer un dato, esta
 * es la respuesta honesta: en vez de suponer qué lleva un producto, se envía
 * el papel que lo dice.
 */
function documento(url, nombre, pie) {
  return {
    type: 'document',
    document: {
      link: url,
      filename: unaLinea(nombre, 60),
      ...(pie ? { caption: corta(pie, 1024) } : {}),
    },
  };
}
...
function aTexto(mensaje) {
  ...
  if (mensaje.type === 'document') {
    return [mensaje.document.caption, `[adjunto: ${mensaje.document.filename}]`]
      .filter(Boolean).join('\n');
  }
...
module.exports = {
  ...
  unaLinea,
  accesosRapidos, menuCategorias, paginaDeProductos, INTERACTIVO, corta, documento,
};
