...
/**
 * Respuesta a una pregunta sobre un producto.
 *
 * Todo lo que se afirma sale LITERAL del documento del fabricante; lo que no
 * está validado no se dice. Cuando no hay dato, el PDF ES la respuesta, no un
 * premio de consolación: se adjunta y se dice por qué.
 *
 * El botón de comprar va al final. Quien pregunta qué lleva un producto no
 * está comprando todavía, pero tampoco hay que obligarle a repetir el nombre
 * después.
 */
function respuestaFicha(producto, res, { url = null, campo = null } = {}) {
  const L = [`*${nombreCompleto(producto)}*`, ''];

  if (res.hay) {
    const partes = res.campos || [{ etiqueta: res.etiqueta, texto: res.texto }];
    for (const c of partes) {
      L.push(`*${c.etiqueta}*`);
      L.push(c.texto);
      L.push('');
    }
    L.push('_Texto literal de la ficha técnica del fabricante._');
  } else {
    L.push(fichas.textoSinDato(res, { nombreProducto: null }));
  }

  const pantallas = [formato.texto(L.join('\n').trim())];
  if (url) {
    pantallas.push(formato.documento(url, `ficha-${res.codigo}.pdf`,
      'Ficha técnica del fabricante.'));
  }
  pantallas.push(formato.botones('¿Te lo añado al pedido?', [
    { id: `elegir_${producto.codigo}`, titulo: '🛒 Añadir' },
...
/* ---- checkout ----------------------------------------------------------- */
/**
 * Resumen final y la decisión. **Siempre con botones**: dejar solo un texto
 * fue lo que impidió a Fernando cerrar un pedido, porque no había forma de
 * confirmarlo.
 */
function pedirConfirmacion(carrito, cliente) {
  return [formato.texto(pedidoLib.textoResumen(carrito, cliente)),
    formato.botones('¿Confirmas el pedido?', [
      { id: 'confirmar_pedido', titulo: '✅ Confirmar' },
      { id: 'modificar_carrito', titulo: '✏️ Modificar' },
      { id: 'cancelar_pedido', titulo: '❌ Cancelar' }])];
}

/** Pedido cerrado. Ya no es editable, y se dice con claridad. */
function pedidoConfirmado(pedido) {
  return [formato.botones(
    `✅ *Pedido confirmado*\n\nHemos recibido tu pedido.\n*N.º ${pedido.id}*\n\n`
    + 'Chacón Alcántara lo revisará, confirmará el importe final y realizará '
    + 'el envío lo antes posible.',
    [{ id: 'nuevo_pedido', titulo: '🛒 Otro pedido' },
     { id: 'ir_home', titulo: '🏠 Menú' }])];
}

/** Ya hay un borrador con productos: se pregunta, no se pisa. */
function borradorEnCurso(lineas, { paraNuevo = false } = {}) {
  const n = lineas.length;
  return [formato.botones(
    paraNuevo
      ? `Tienes un pedido sin confirmar con ${n} producto${n === 1 ? '' : 's'}.\n`
        + 'Si empiezas otro, guardaremos este como no finalizado.'
      : `Tienes un pedido en curso con ${n} producto${n === 1 ? '' : 's'}.\n¿Qué quieres hacer?`,
    [{ id: 'continuar_pedido', titulo: '🛒 Continuar' },
     { id: 'nuevo_pedido_si', titulo: '✨ Empezar nuevo' }])];
}

function confirmarCancelacion() {
  return [formato.botones('¿Quieres cancelar este pedido?', [
    { id: 'cancelar_si', titulo: '✅ Sí, cancelar' },
    { id: 'ver_carrito', titulo: '❌ No, continuar' }])];
}

/** Un pedido confirmado no se reabre en silencio. */
function pedidoYaConfirmado() {
  return [formato.botones(
    'Ese pedido ya está confirmado y no se puede modificar.\n¿Quieres hacer otro?',
    [{ id: 'nuevo_pedido', titulo: '✨ Nuevo pedido' },
     { id: 'hablar_fernando', titulo: '👤 Hablar con Fernando' }])];
}

/* ---- privacidad --------------------------------------------------------- */
/** Primera capa: corta, con enlace, y una acción explícita. */
function avisoPrivacidad(texto) {
  return [formato.botones(texto, [
    { id: 'privacidad_si', titulo: '✅ Continuar' },
    { id: 'privacidad_no', titulo: '❌ Ahora no' },
  ])];
}

/** Ha dicho que no. Se para, sin insistir. */
function privacidadRechazada() {
  return [formato.botones(
    'Sin problema. No continuaremos con el pedido por este canal.',
    [{ id: 'hablar_fernando', titulo: '👤 Hablar con Fernando' }])];
}

/** Marketing: aparte, y siempre después de que el pedido esté resuelto. */
function pedirMarketing() {
  return [formato.botones(
    '¿Quieres recibir también ofertas y novedades de Chacón por WhatsApp?\n'
    + '_Puedes decirme que no y seguir haciendo pedidos igual._',
    [{ id: 'marketing_si', titulo: '✅ Sí, quiero' },
     { id: 'marketing_no', titulo: 'No, gracias' }])];
}
 ...
 */
function negocioNoEncontrado(nombre, { altaLibre = false } = {}) {
  if (altaLibre) {
    // Modo demo: se puede continuar con el nombre que ha dado.
    return [formato.botones(
      `No te tengo registrado como «${nombre}». ¿Seguimos así de momento?`,
      [{ id: 'alta_negocio', titulo: `✅ Sí, continuar` },
...
/** Confirmación antes de vincular. Nunca se ata en silencio. */
function confirmarNegocio(clienteAgenda) {
  const L = ['He encontrado:', '', `*${clienteAgenda.legal_name}*`];
  if (clienteAgenda.display_name && clienteAgenda.display_name !== clienteAgenda.legal_name) {
    L.push(`_${clienteAgenda.display_name}_`);
  }
  L.push('');
  L.push('¿Es tu negocio?');
  return [formato.botones(L.join('\n'), [
    { id: `cliente_si:${clienteAgenda.customer_code}`, titulo: '✅ Sí, es este' },
    { id: 'reintentar_negocio', titulo: '🔎 Buscar otro' },
    { id: 'hablar_fernando', titulo: '👤 Hablar con Fernando' },
  ])];
}

/** Varios de la agenda: se enseñan y se elige. Nunca se decide por parecido. */
function elegirNegocio(candidatos) {
  return [formato.lista('He encontrado estos clientes parecidos. ¿Cuál es el tuyo?', {
    boton: 'Elegir',
    secciones: [{
      titulo: 'Clientes',
      filas: candidatos.slice(0, 3).map((c) => ({
        id: `cliente_si:${c.customer_code}`,
        titulo: formato.unaLinea(c.display_name || c.legal_name, 24),
        descripcion: formato.unaLinea(c.legal_name, 72),
      })).concat([{ id: 'reintentar_negocio', titulo: 'Ninguno de estos' }]),
    }],
  })];
}

/** No está en la agenda. NO se inventa un cliente de Chacón. */
function negocioFueraDeAgenda(nombre) {
  return [formato.botones(
    `No encuentro «${nombre}» entre los clientes registrados de Chacón.\n`
    + 'Fernando puede revisarlo y vincularlo para que puedas seguir.',
    [{ id: 'reintentar_negocio', titulo: '✏️ Otro nombre' },
     { id: 'hablar_fernando', titulo: '👤 Avisar a Fernando' }])];
}
...
module.exports = {
  ...
  avisoPrivacidad, privacidadRechazada, pedirMarketing,
  confirmarNegocio, elegirNegocio, negocioFueraDeAgenda,
  pedirConfirmacion, pedidoConfirmado, borradorEnCurso, confirmarCancelacion,
  pedidoYaConfirmado,
  verCarrito, editarCarrito, verOfertas, misHabituales, escalarAFernando,
  preciosDe, lineasDePrecio, nombreCompleto, bonito, tituloFila, eur, MAX_OPCIONES, respuestaFicha,
};
