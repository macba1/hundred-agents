...
/** Peso legible: 5 kg, no 5,000 kg. Los ceros de relleno estorban. */
const kg = (n) => (n === null || n === undefined ? null
...
async function precioDe(producto, { cantidad = null, unidad = null } = {}) {
  ...
  /* Se nombra el TRAMO que corresponde, no "Tarifa 1" siempre. Con el motor
     de tramos, una caja se cotiza a la tarifa 3: llamar "Tarifa 1" a ese
     precio sería decirle al cliente algo que no es. */
  const tramo = vig.tier_label
    ? { etiqueta: vig.tier_label.replace(/ OFERTA$/, ''), numero: vig.tier }
    : { etiqueta: null, numero: '1' };
  const comoTarifa = tramo.etiqueta
    ? `Tarifa ${tramo.numero} (${tramo.etiqueta.toLowerCase()})`
    : 'Tarifa 1';
  ...
  if (vig.es_oferta) {
    ...
    if (vig.precio_normal_kg !== null && vig.precio_normal_kg !== undefined) {
      L.push(`Su precio habitual de ${comoTarifa} es ${eur(vig.precio_normal_kg)} €/kg.`);
    }
    if (vig.condiciones) L.push(`Condiciones: ${vig.condiciones}`);
    if (vig.vigencia?.hasta) L.push(`Válida hasta el ${vig.vigencia.hasta}.`);
  } else {
    L.push(`El precio de ${comoTarifa} de ${producto.descripcion} es `
      + `${eur(vig.precio_kg)} €/kg, sin IVA.`);
  }
  // Sin cantidad se ha dado la pieza: conviene decir que por caja baja.
  if (vig.sin_cantidad) {
    L.push('Ese es el precio por pieza; si lo pides por cajas te sale más barato: dime cuántas.');
  }
  ...
  if (ficha.peso_und_kg) detalles.push(`${kg(ficha.peso_und_kg)} kg por unidad`);
...
async function consultarPrecio(consulta, { cantidad = null, unidad = null } = {}) {
  ...
  // Los nombres de tipo son los que devuelve `catalogo.buscar`: si aquí se
  // pone uno que no existe, la consulta cae al camino de "aclaración" y el
  // agente responde `undefined`. Ya pasó una vez.
  const IDENTIFICAN = ['codigo_exacto', 'ean_exacto'];
  const exacto = r.candidatos.length === 1 || IDENTIFICAN.includes(r.tipo);
  if (!exacto) {
    return {
      ...
      respuesta_exacta: 'Tengo varios productos que encajan. ¿Cuál de estos es?\n'
        + r.candidatos.slice(0, 6).map((p) => `• [${p.codigo}] ${p.descripcion}`).join('\n'),
