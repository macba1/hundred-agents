...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Busca el negocio en la agenda oficial de clientes de Chacón. '
        + 'NO da de alta a nadie: si no está, dilo y ofrece avisar a Fernando.',
      parameters: {
        type: 'object',
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Ficha técnica del fabricante: ingredientes, alérgenos, conservación, '
        + 'denominación, presentación o modo de empleo. Devuelve la respuesta EXACTA que '
        + 'debes dar y, si procede, el PDF a adjuntar. Úsala para cualquier pregunta sobre '
        + 'qué lleva o cómo se conserva un producto.',
      parameters: {
        type: 'object',
        properties: {
          producto_id: { type: 'string' },
          campo: {
            type: 'string',
            enum: ['ingredientes', 'alergenos', 'conservacion',
                   'denominacion', 'presentacion', 'modo_empleo'],
          },
        },
        required: ['producto_id', 'campo'],
      },
    },
  },
  {
...
async function ejecutar(ctx, nombre, args) {
  ...
  if (nombre === 'identificar_tienda') {
    /* El agente NO da de alta clientes. Solo la agenda oficial de Chacón dice
       quién es cliente, y el flujo guiado es quien la consulta y vincula.
       Antes esta herramienta llamaba a `crearCliente` y creaba fichas que no
       existían en la agenda: así apareció un cliente inventado en producción. */
    const agenda = require('./clientes');
    const r = agenda.buscar(args.nombre || '');
    if (r.tipo === 'exacto' || r.tipo === 'probable') {
      return { encontrado: true, requiere_confirmacion: true,
               legal_name: r.cliente.legal_name,
               nota: 'NO lo des por identificado: el cliente tiene que confirmarlo. '
                 + 'Dile el nombre que has encontrado y pregúntale si es su negocio.' };
    }
    if (r.tipo === 'varios') {
      return { encontrado: false, varios: r.candidatos.map((c) => c.legal_name),
               nota: 'Hay varios parecidos. Pregúntale cuál es. No elijas tú.' };
    }
    return { encontrado: false,
             nota: 'No está entre los clientes de Chacón. Dile que Fernando puede revisarlo. '
               + 'NO inventes un cliente ni le des de alta.' };
  }
  ...
  if (nombre === 'consultar_ficha_tecnica') {
    const p = catalogo.porId(args.producto_id);
    if (!p) return { error: 'producto_inexistente' };
    const r = await fichas.consultar(p.codigo, args.campo);
    const url = fichas.urlPdf(p.codigo);

    if (r.hay) {
      /* El texto sale LITERAL del documento del fabricante. El modelo puede
         presentarlo, pero no puede reescribirlo: en alérgenos, una paráfrasis
         es un dato distinto. */
      return {
        respuesta_exacta: `*${r.etiqueta}* — ${p.descripcion}\n${r.texto}`,
        dato_disponible: true,
        adjuntar_pdf: url || null,
        nota: 'Reproduce `respuesta_exacta` PALABRA POR PALABRA. No lo resumas, '
          + 'no lo traduzcas y no añadas conclusiones propias.',
      };
    }

    ctx.consultasFichaSinDato.push({ producto_id: p.id, codigo: p.codigo,
                                     campo: args.campo, motivo: r.motivo });
    return {
      respuesta_exacta: fichas.textoSinDato(r, { nombreProducto: p.descripcion }),
      dato_disponible: false,
      adjuntar_pdf: url || null,
      nota: 'Responde EXACTAMENTE esto. NO deduzcas el dato por el tipo de producto, '
        + 'por la marca ni por otros productos parecidos.',
    };
  }
...
function systemPrompt(ctx) {
  ...
  return [
    ...
    'Si preguntan qué lleva algo, si es apto para celíacos, cómo se conserva o piden la',
    'ficha técnica, llama a `consultar_ficha_tecnica` y reproduce `respuesta_exacta`',
    '**palabra por palabra**. Ese texto es el del documento del fabricante: no lo resumas,',
    'no lo traduzcas y no saques conclusiones de él.',
    '',
    'En particular, **nunca digas que un producto NO lleva algo** porque no aparezca en la',
    'lista de ingredientes. Las trazas por contaminación cruzada van en otro apartado del',
    'documento. Enseña la lista y que la lea la tienda.',
    '',
    'Si la pregunta no dice de qué producto es —"y el queso?"— mira la conversación para',
    'saber de cuál se habla. Si hay varios posibles, enseña la lista y pregunta cuál: no',
    'elijas tú. Y si no tienes el dato, dilo y manda el PDF; eso es una respuesta correcta.',
    '',
    '## Lo que NUNCA haces',
    '- Inventar productos, stock, pesos, precios, ofertas o información de alérgenos.',
    '- Afirmar que algo no contiene un ingrediente o un alérgeno.',
...
async function responder({ telefono, texto, historial = [], claveIdempotencia = null, urlPanel = null }) {
  ...
  const ctx = {
    ...
    consultasAlergenoSinDato: [], consultasFichaSinDato: [], pedidoConfirmado: null,
  };
  ...
  return {
    ...
    consultas_ficha_sin_dato: ctx.consultasFichaSinDato,
