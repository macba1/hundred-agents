...
/* Saludos al principio y la intención al final. La gente se presenta y pide
   en la misma frase: "hola soy carnicería el chino y quiero hacer un pedido".
   Sin recortar los dos extremos, el nombre se pierde entre el ruido. */
const SALUDO_INICIAL = /^\s*(hola|buenas|buenos dias|buenas tardes|buenas noches|hey)\b[\s,.:!]*/i;
const INTENCION_FINAL = new RegExp(
  '\\s*(?:,|\\.|;)?\\s*\\b(?:y\\s+)?(?:quiero|queremos|queria|queriamos|querria|'
  + 'necesito|necesitamos|me\\s+gustaria|nos\\s+gustaria|voy\\s+a|vamos\\s+a|'
  + 'deseo|deseamos|para\\s+(?:hacer|pedir))\\b.*$', 'i');
...
function nombreDeNegocio(texto) {
  ...
  t = t.replace(SALUDO_INICIAL, '').trim();
  t = t.replace(INTENCION_FINAL, '').trim();
  if (!t) return null;
 ...
 */
async function vincular(clienteAgenda, telefono, {
  center = null, estadoCentro = null, nivel = confianza.NIVELES.AUTOIDENTIFICADO } = {}) {
  const existentes = await repo.listarClientes();
  let ficha = existentes.find((c) => c.customer_code === clienteAgenda.customer_code);
  const tel = confianza.normalizarTelefono(telefono) || telefono;
  ...
  /* Queda constancia de que este número ha dicho ser esta tienda, aunque no
     llegue a pedir: alguien probando nombres hasta acertar no deja otra
     huella. */
  await repo.anotarReclamacion(tel, clienteAgenda.customer_code).catch(() => {});

  /* La confianza NUNCA baja sola. Si Chacón ya verificó este teléfono, que
     vuelva a identificarse por su nombre no lo devuelve a sin verificar. */
  ficha.telefonos_verificados = ficha.telefonos_verificados || [];
  const yaVerificado = ficha.telefonos_verificados.includes(tel);
  const subeANivel = nivel !== confianza.NIVELES.AUTOIDENTIFICADO;
  ficha.link_trust = (yaVerificado || subeANivel)
    ? (subeANivel ? nivel : ficha.link_trust) : confianza.NIVELES.AUTOIDENTIFICADO;
  if (subeANivel && !yaVerificado) ficha.telefonos_verificados.push(tel);
  ...
  /* `estado` habla del NEGOCIO —está en la agenda oficial de Chacón— y
     `link_trust` habla del TELÉFONO. Confundirlos fue el agujero: una tienda
     real en la agenda no demuestra que quien escribe sea ella. */
  ficha.estado = 'verificado';
  ficha.link_status = 'confirmed';
  ficha.link_source = nivel === confianza.NIVELES.AGENDA
    ? 'agenda_telefono' : 'whatsapp_self_identification';
  ...
  console.log('[chacon][evento] customer_linked tel=%s code=%s center=%j version=%s confianza=%s',
    telefono, ficha.customer_code, center, ficha.agenda_version, ficha.link_trust);
  return ficha;
}
