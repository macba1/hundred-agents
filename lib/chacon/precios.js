/* ============================================================
   ...
   **La tarifa la elige código determinista, nunca el modelo.**

   Alcance del MVP: Chacón confirma que el precio del PDF **es la Tarifa 1**.
   El MVP trabaja exclusivamente con ella, así que ya no hay que adivinar el
   tramo y el cálculo deja de bloquearse por eso. Las tarifas 2 a 8 siguen
   modeladas —`NIVELES`, `elegirNivel`, `aplicarUmbrales`— para cuando Chacón
   entregue sus umbrales y su tabla de precios, pero no intervienen en la
   conversación.
   ============================================================ */

/** Única tarifa del MVP. El PDF entregado es la Tarifa 1. */
const TARIFA_MVP = 1;
...
/**
 * Calcula la línea con la Tarifa 1.
 *
 * Dos bloqueos distintos, y no hay que confundirlos:
 *   - `precioValido === false` -> no hay un precio que se pueda afirmar
 *     (los 19 códigos sin resolver, o la promoción sin condiciones).
 *   - `peso_estimado_kg === null` -> hay precio, pero no se puede estimar el
 *     importe porque no se sabe cuánto pesa.
 *
 * Ninguno de los dos impide pedir el producto: impiden **afirmar una cifra**.
 *
 * `precioAplicado` permite inyectar el precio ya resuelto por un
 * administrador o el precio de una oferta validada, sin tocar el catálogo.
 */
function calcularLinea({ producto, cantidad, unidadPedido, reglas = null, precioAplicado = null }) {
  const conv = convertir({
    cantidad, unidadPedido,
    und_caja: producto.und_caja, peso_und_kg: producto.peso_und_kg,
  });
  // En el MVP la tarifa es siempre la 1. `elegirNivel` sigue disponible para
  // cuando existan las tarifas 2-8, pero no decide nada aquí.
  const nivel = { nivel: TARIFA_MVP, determinado: true, motivo: 'MVP: Tarifa 1 única' };

  const bloqueos = [];
  // Un precio inyectado y validado gana: significa que alguien lo resolvió.
  const resuelto = precioAplicado && Number.isFinite(precioAplicado.precio_kg);
  if (!resuelto && producto.bloqueado_para_calculo_precio) {
  ...
  if (!resuelto && (producto.tarifa === null || producto.tarifa === undefined)) bloqueos.push('sin_precio');

  const precioBloqueado = bloqueos.some((b) => b !== 'peso_desconocido');
  const precio_kg = precioBloqueado ? null : (resuelto ? precioAplicado.precio_kg : producto.tarifa);
  const puedeCalcular = precio_kg !== null && conv.peso_estimado_kg !== null;
  ...
  return {
    ...
    precio_pendiente_de_confirmacion: precioBloqueado,
    es_oferta: !!(precioAplicado && precioAplicado.es_oferta),
    origen_precio: precioBloqueado ? null
      : (resuelto ? (precioAplicado.origen || 'resuelto_por_administrador') : 'tarifa_1_pdf'),
...
module.exports = { NIVELES, TARIFA_MVP, tramosDesdeCatalogo, convertir, elegirNivel,
                   calcularLinea, totalizar, redondear };
