...
const { normalizarTelefono } = require('./confianza');
...
function cargar() {
  ...
  /* Índice por teléfono, cuando Chacón lo aporte. Un número que apareciera en
     dos tiendas NO resuelve a ninguna: identificar mal a quien pide es peor
     que preguntarle. Se marca para que salga en el panel. */
  const porTelefono = new Map();
  const telefonosDuplicados = new Map();
  for (const c of clientes) {
    for (const bruto of c.phones || []) {
      const tel = normalizarTelefono(bruto);
      if (!tel) continue;
      const ya = porTelefono.get(tel);
      if (ya && ya.customer_code !== c.customer_code) {
        const l = telefonosDuplicados.get(tel) || [ya.customer_code];
        if (!l.includes(c.customer_code)) l.push(c.customer_code);
        telefonosDuplicados.set(tel, l);
        porTelefono.delete(tel);
        continue;
      }
      if (!telefonosDuplicados.has(tel)) porTelefono.set(tel, c);
    }
  }
  _cache = { estado, version, clientes, porCodigo, porTelefono, telefonosDuplicados };
  return _cache;
}
...
/**
 * La tienda a la que Chacón asigna este teléfono, si lo ha dicho.
 *
 * Es la única identificación que no depende de lo que cuente quien escribe,
 * así que es la única que puede darse por buena sin verificación posterior.
 */
function porTelefonoAgenda(telefono) {
  const tel = normalizarTelefono(telefono);
  if (!tel) return null;
  return cargar().porTelefono.get(tel) || null;
}

/** Números que la agenda asigna a más de una tienda: hay que corregirlos. */
const telefonosAmbiguos = () => [...cargar().telefonosDuplicados.entries()]
  .map(([telefono, codigos]) => ({ telefono, codigos }));

/** Cuántas tiendas traen teléfono. Sin esto no se sabe qué cobertura hay. */
const conTelefono = () => cargar().clientes.filter((c) => (c.phones || []).length).length;
 ...
 */
/* Palabras con las que la gente presenta su código: "mi código es 90014",
   "soy el cliente 340". Sin esto el código solo servía escrito a secas, y en
   producción alguien lo dio en una frase y no se le encontró. */
const PIDE_CODIGO = /\b(codigo|cod|cliente|numero|num|ref|referencia|cuenta)\b/i;

/** Busca en la frase un token que sea un código de cliente real. */
function codigoEnFrase(texto) {
  const tokens = String(texto || '').match(/[A-Za-z0-9-]{1,12}/g) || [];
  const hits = [];
  for (const t of tokens) {
    const c = porCodigo(t);
    if (c && !hits.some((x) => x.customer_code === c.customer_code)) hits.push(c);
  }
  return hits;
}
...
function buscar(texto, { maximo = 3 } = {}) {
  ...
  /* Si menciona explícitamente su código, se busca dentro de la frase. Solo
     cuando lo dice, para que "carnicería 3" no acabe resolviendo al cliente
     número 3 por accidente. */
  if (PIDE_CODIGO.test(bruto)) {
    const hits = codigoEnFrase(bruto);
    if (hits.length === 1) return { tipo: 'exacto', por: 'codigo', cliente: hits[0] };
    if (hits.length > 1) return { tipo: 'varios', candidatos: hits.slice(0, maximo) };
  }
  ...
  if (!puntuados.length) {
    /* Último recurso: quizá dio el código sin decir que lo era. Se exigen 3
       cifras o más: si no, "carnicería 3" resolvería al cliente número 3, que
       es justo el tipo de acierto casual que no se puede permitir. */
    const hits = codigoEnFrase(bruto).filter((c) => String(c.customer_code).length >= 3);
    if (hits.length === 1) return { tipo: 'exacto', por: 'codigo_en_frase', cliente: hits[0] };
    return { tipo: 'nada', motivo: 'sin_coincidencias' };
  }
...
function resumen() {
  ...
  return {
    ...
    telefonos_ambiguos: telefonosAmbiguos(),
    versiones: estado.versiones || [],
  };
}

module.exports = {
  buscar, porCodigo, porTelefonoAgenda, telefonosAmbiguos, conTelefono,
  codigoEnFrase, centroDe, todos, disponible, versionActiva, resumen,
