...
function componerMensaje(pedido, { urlPanel = null } = {}) {
  ...
  /* El aviso de teléfono sin verificar va ARRIBA del todo, antes incluso del
     número de pedido. Esto se lee en un móvil: lo que va al final se lee a
     veces, y este es el aviso que decide si el pedido se sirve o no. */
  const avisoConfianza = confianza.avisoParaChacon(pedido.verificacion);
  if (avisoConfianza.length) { L.push(...avisoConfianza, ''); }
