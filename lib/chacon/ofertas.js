...
async function precioVigente(producto, { cantidad = null, unidad = null, ahora = new Date() } = {}) {
  /* Motor de tarifas v2: ocho tramos con sus ofertas, versionado y aprobado.
     ...
     Precedencia, y no es un detalle: los precios los manda la VERSIÓN de
     tarifas, pero una desactivación explícita desde el panel se respeta. Si
     no, el panel diría "oferta desactivada" y el agente seguiría aplicándola:
     un estado falso, que es peor que no tener el botón. */
  /* Sin cantidad se usa el tramo 1 (PIEZA), que es lo que responde "¿a cuánto
     está el 6305?". Antes esta rama exigía cantidad Y unidad, así que la
     pregunta más frecuente caía al camino viejo y seguía contestando "tengo
     dos precios registrados" aunque la tarifa tuviera respuesta y oferta. */
  if (tarifas.disponible()) {
    const decision = await get(producto.id);
    const ofertaSuprimida = decision.offer_price_per_kg === null
      && decision.offer_active === false
      && !!decision.offer_validated_by;

    const sinCantidad = cantidad === null || unidad === null;
    const r = sinCantidad
      ? (() => {
        const p = tarifas.precioDe(producto.codigo, '1');
        return p.encontrado
          ? { ok: true, precio: p, tramo: { tier: '1', determinado: true,
                                            motivo: 'consulta sin cantidad: se da la PIEZA' } }
          : { ok: false, error: p.motivo };
      })()
      : tarifas.precioParaCantidad(producto.codigo,
    ...
    if (r.ok && !r.precio.promotion_rule_required) {
      ...
      // Con la oferta suprimida a mano se cobra el precio normal del tramo.
      const usarOferta = r.precio.es_oferta && !ofertaSuprimida;
      const aplicado = usarOferta ? r.precio.aplicado_e4
        : (r.precio.normal_e4 ?? r.precio.aplicado_e4);
      return {
        precio_kg: aplicado / tarifas.ESCALA,
        precio_e4: aplicado,
        es_oferta: usarOferta,
        oferta_suprimida_por_administrador: r.precio.es_oferta && ofertaSuprimida
          ? (decision.offer_validated_by || true) : false,
        origen: `tarifa_${usarOferta ? r.precio.tariff_code : r.precio.tier}_v${r.precio.catalog_version}`,
