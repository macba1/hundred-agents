/* ============================================================
   ...
   Keys (all namespaced per client so tenants never collide):
     ...
     wa:{client}:perfil:{phone}    JSON {nombre, ultimo_pedido}  TTL 180d
...
// El perfil sobrevive a la sesión: es lo que permite saludar por su nombre a
// alguien que vuelve la semana siguiente.
const PERFIL_TTL = Number(process.env.WA_PERFIL_TTL || 60 * 60 * 24 * 180);  // 180d
...
const kPerfil = (c, phone) => `wa:${c}:perfil:${phone}`;
...
/** Cierra la conversación SIN borrar el perfil: el cliente vuelve a ser
 *  saludado como recurrente, pero empieza charla nueva. */
async function resetSession(c, phone) {
  const cli = await client();
  const borradas = await cli.del(kSession(c, phone));
  return { session_borrada: borradas > 0, perfil_conservado: true };
}

/* ---- perfil del cliente ------------------------------------- */
async function getPerfil(c, phone) {
  const cli = await client();
  const raw = await cli.get(kPerfil(c, phone));
  if (!raw) return null;
  try { return JSON.parse(raw); } catch { return null; }
}

/** Mezcla campos nuevos sobre el perfil existente. */
async function savePerfil(c, phone, patch) {
  const cli = await client();
  const actual = (await getPerfil(c, phone)) || { pedidos: 0 };
  const perfil = { ...actual, ...patch, actualizado: new Date().toISOString() };
  await cli.set(kPerfil(c, phone), JSON.stringify(perfil), { EX: PERFIL_TTL });
  return perfil;
}
...
/* ---- cortacircuitos de respuestas --------------------------- */
/**
 * Cuenta respuestas del agente por teléfono y hora. Si algo vuelve a
 * disparar mensajes en bucle, esto lo corta antes de gastar OpenAI y antes
 * de llenarle el WhatsApp a nadie.
 */
async function bumpRespuesta(c, phone) {
  const cli = await client();
  const hora = new Date().toISOString().slice(0, 13); // YYYY-MM-DDTHH
  const key = `wa:${c}:replies:${phone}:${hora}`;
  const n = await cli.incr(key);
  if (n === 1) await cli.expire(key, 3600);
  return n;
}
...
async function purge(c, scope, esTest) {
  ...
  for (const phone of telefonos) {
    ...
    await cli.del(kPerfil(c, phone));
    out.claves_borradas += 2;
    for (let d = 0; d < 3; d += 1) {
...
module.exports = {
  ...
  bumpAudio, purge, resetSession, getPerfil, savePerfil, bumpRespuesta,
