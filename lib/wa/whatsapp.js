...
async function sendText(client, to, body) {
  ...
  if (!r.ok) {
    console.error('[wa] envío falló', destino, r.status, (await r.text()).slice(0, 400));
    return false;
  }
  return true;
}
...
module.exports = { GRAPH, API_VERSION, sendText, downloadMedia, verifySignature, token, normalizarDestino };
