...
const AUDIO_MAX_PER_DAY = Number(process.env.AUDIO_MAX_PER_DAY || 30);
 ...
 * @param {object} [opts] {tools, model} — la demo web de la landing usa otras
 *   herramientas (no toca Redis ni Meta) y otro modelo. Sin opts se comporta
 *   exactamente igual que antes para el webhook de WhatsApp.
 */
...
async function runAgent(client, phone, userContent, memoryText, session) {
  ...
  // El perfil se carga en CADA turno. Cargarlo solo en el primero dejaba al
  // modelo sin saber qué era "lo mismo" justo cuando el cliente lo pedía.
  const perfil = await store.getPerfil(client.clave, phone);
  ...
  const iterar = async () => {
    ...
    for (const tc of calls) {
      ...
      if (tc.function.name === 'registrar_pedido' && result && result.folio) {
        ...
        // registrar_pedido escala solo cuando lleva algo fuera de carta.
        if ((result.fuera_de_carta || []).length) escaloEsteTurno = true;
      }
      if (tc.function.name === 'escalar_humano' && result && result.escalado) {
        escaloEsteTurno = true;
      }
  ...
  // Guardarraíl: prometió consultarlo con el equipo y no escaló a nadie. El
  // cliente se queda esperando una respuesta que nadie va a dar.
  // El costo de envío se excluye a propósito: ahí "el equipo te confirma" es
  // literal y no requiere escalar.
  // Raíces verbales, no conjugaciones: falla con "pasé", "consulto", "checo".
  const PROMESA = /(confirm|consult|chec|pregunt|revis|verific|pas)[a-záéíóúñ]*\b[^.\n]{0,40}\bequipo/i;
  const ES_ENVIO = /env[ií]o|monto exacto|zona/i;
  const prometioEquipo = PROMESA.test(finalText) && !ES_ENVIO.test(finalText);
  if (prometioEquipo && !escaloEsteTurno) {
    console.error('[wa] prometió consultar al equipo sin escalar. phone=%s', phone);
    messages.push({
      role: 'system',
      content:
        'ALTO: le dijiste al cliente que lo consultarías con el equipo, pero no ' +
        'llamaste a escalar_humano, así que nadie se enteró y se va a quedar ' +
        'esperando. Llama AHORA a escalar_humano explicando exactamente qué pidió, ' +
...
async function transcribirAudio(client, phone, mediaId) {
  ...
  const tope = client.audio_max_dia || AUDIO_MAX_PER_DAY;
  const usados = await store.bumpAudio(client.clave, phone, dia);
  if (usados > tope) {
    console.log('[wa] audio: límite diario alcanzado (%s/%s)', usados, tope, client.clave, phone);
 ...
 * @param {object} [cls] clasificación de lib/wa/inbound. El webhook ya filtró
 *   lo no accionable; aquí solo llegan 'agente' y 'no_soportado'.
 */
async function handleMessage(client, m, cls) {
  const phone = m.from;
  if (!phone) return;
  const tipo = m.type || 'unknown';

  // Falla-cerrado: si el webhook no lo marcó como contenido de usuario, no se
  // llama a OpenAI y no sale ningún mensaje.
  if (cls && cls.accion === 'no_soportado') {
    await wa.sendText(
      client, phone,
  ...
  if (cls && cls.accion !== 'agente') {
    console.log('[wa] no accionable, sin respuesta:', tipo);
    return;
  }
  ...
  try {
    ...
    // Cualquier otro tipo ya se filtró en el webhook. Si llegara algo aquí,
    // se registra y se calla: nunca un mensaje saliente por un payload raro.
    console.warn('[wa] tipo inesperado en handleMessage, sin respuesta:', tipo);
  } catch (err) {
