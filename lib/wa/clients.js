...
function systemPrompt(client, opts = {}) {
  ...
  if (!opts.primerMensaje) {
    ...
    // Instrucción ÚNICA para este caso. Antes convivía con la del saludo
    // genérico y el modelo obedecía la genérica, que decía "tal cual".
    const u = perfil.ultimo_pedido || {};
    partes.push(
      '## PRIMER MENSAJE DE UN CLIENTE QUE YA TE HA ESCRITO ANTES\n' +
      (perfil.nombre ? `Se llama **${perfil.nombre}**. ` : '') +
      (perfil.pedidos ? `Lleva ${perfil.pedidos} pedido(s) con nosotros. ` : '') +
      (u.resumen ? `\nSu último pedido (${u.folio}) fue: ${u.resumen}` +
        (u.total != null ? ` — total $${u.total}.` : '.') : '') +
      '\n\n**NO uses el saludo con las 3 opciones.** Para este cliente el saludo es ' +
      'otro: salúdalo por su nombre y ofrécele lo mismo de la última vez, en una o ' +
      'dos líneas. Por ejemplo:\n' +
      `"¡Hola${perfil.nombre ? ', ' + perfil.nombre : ''}! ¿Te preparo lo mismo de la última vez?"\n\n` +
      'Si dice que sí, **el pedido es EXACTAMENTE el que te acabamos de dar arriba**, ' +
      'con esos mismos platillos y esas mismas variantes ya elegidas: no vuelvas a ' +
      'preguntar "¿verdes o rojos?" si arriba dice "Verdes", y no cambies un platillo ' +
      'por otro. Copia la lista de arriba, no la inventes. ' +
      'Vuelve a consultar el catálogo para **re-verificar cada precio** (pueden haber ' +
      'cambiado), y sigue el flujo normal desde el paso de "¿algo más?". ' +
      'No des por hecho el pago ni la entrega: pregúntalos.\n' +
      ...
      'No recites el pedido anterior entero salvo que te lo pida.'
    );
  } else {
    partes.push(
