...
function loadOne(clave) {
  ...
  const client = {
    ...
    // Demo web (lib/wa/demo.js + api/demo-chat.js). Sin esta bandera el cliente
    // NO queda expuesto en la web aunque exista aquí: un cliente de WhatsApp
    // real se atiende solo por su webhook. Por defecto false.
    demo_web: !!cfg.demo_web,
    base_web: cfg.base_web || '',
    sugerencias_inicio: Array.isArray(cfg.sugerencias_inicio)
      ? cfg.sugerencias_inicio.map(String).filter(Boolean)
      : [],
