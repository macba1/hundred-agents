...
function systemPrompt(client, opts = {}) {
  ...
  if (opts.primerMensaje && opts.perfil && (opts.perfil.nombre || opts.perfil.ultimo_pedido)) {
    const p = opts.perfil;
    const u = p.ultimo_pedido || {};
    partes.push(
      '## CLIENTE QUE YA TE HA ESCRITO ANTES\n' +
      (p.nombre ? `Se llama **${p.nombre}**. ` : '') +
      (p.pedidos ? `Lleva ${p.pedidos} pedido(s) con nosotros. ` : '') +
      (u.resumen ? `\nSu último pedido (${u.folio}) fue: ${u.resumen}` +
        (u.total != null ? ` — total $${u.total}.` : '.') : '') +
      '\n\nSalúdalo por su nombre y ofrécele lo mismo de la última vez, en corto. Por ejemplo:\n' +
      `"¡Hola${p.nombre ? ', ' + p.nombre : ''}! ¿Te preparo lo mismo de la última vez?"\n` +
      'Si dice que sí, **vuelve a consultar el catálogo** para re-verificar cada precio ' +
      '(pueden haber cambiado) y sigue el flujo normal de pedido. ' +
      'Si dice que no, sigue con el saludo de opciones de siempre. ' +
  ...
  partes.push(
    opts.primerMensaje
      ? 'ESTE ES EL PRIMER MENSAJE de la conversación (o el cliente llevaba más ' +
        'de 48h sin escribir). Aplica la sección "Saludo inicial": si el cliente ' +
        'solo saludó, manda el saludo con las 3 opciones tal cual. Si ya trajo un ' +
        'pedido o una pregunta concreta, sáltate el saludo y atiende directo.'
