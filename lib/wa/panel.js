...
function render({ titulo, acento, contactos, total, rows, clientes, clienteSel, token, hora }) {
  ...
  let trs = '';
  for (const r of rows) {
    const hhmm = String(r.ts || '').slice(11, 19);
    ...
    const totalTxt = r.total != null
      ...
      : '';
    trs += `<tr><td>${esc(hhmm)}</td><td>${esc(r.client)}</td><td>…${esc(tel4)}</td>` +
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  <div class="sub">Hundred Agents · Vercel + Redis · auto-refresh 5s · ${esc(hora)}</div>
